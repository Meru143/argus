...
///
/// Supports OpenAI-compatible (`/v1/chat/completions`) and Anthropic
/// (`/v1/messages`) endpoints. The provider is determined by
/// `LlmConfig.provider`.
///
/// # Examples
///
...
impl LlmClient {
    ...
    /// env vars (`OPENAI_API_KEY` or `ANTHROPIC_API_KEY`). When the provider
    /// changes but the model is still the OpenAI default (`gpt-4o`),
    /// auto-switches to `claude-sonnet-4-5` for Anthropic.
    ///
    /// # Errors
    ///
    ...
    pub fn new(config: &LlmConfig) -> Result<Self, ArgusError> {
        ...
        let model = if provider == Provider::Anthropic && config.model == "gpt-4o" {
    ...
    ///
    /// Dispatches to the OpenAI or Anthropic API based on the configured
    /// provider. For Anthropic, system messages are extracted to a top-level
    /// `"system"` field and consecutive user messages are concatenated.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn unknown_provider_returns_error() {
        ...
        assert!(err.contains("openai, anthropic"));
    }
